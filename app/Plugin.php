...
final class Plugin {
    ...
    private function register_key_setup_wizard(): void {
        add_action( 'admin_notices', static function (): void {
            ...
            if ( 'success' === $status ) {
                echo '<div class="notice notice-success is-dismissible"><p><strong>WP License Server:</strong> Encryption key generated and added to <code>wp-config.php</code> successfully.</p></div>';
                return;
            }

            if ( 'no' === $status ) {
                echo '<div class="notice notice-info is-dismissible"><p><strong>WP License Server:</strong> Setup skipped. The plugin will not function until <code>WPLICENSE_ENCRYPTION_KEY</code> is defined. See the <a href="https://example.com/docs">setup guide</a> for manual instructions.</p></div>';
                return;
            }

            $error_messages = [
                'not_found'        => 'wp-config.php could not be found.',
                'not_writable'     => 'wp-config.php is not writable by the web server.',
                'read_error'       => 'Could not read wp-config.php.',
                'write_error'      => 'Could not write to wp-config.php.',
                'marker_not_found' => 'Could not find the right location in wp-config.php.',
                'backup_failed'    => 'Could not create wp-config.php backup. No changes were made.',
                'invalid_key'      => 'Generated key contains unexpected characters. This should not happen.',
                'dir_not_writable' => 'The wp-config.php directory is not writable for atomic replace.',
            ];
            if ( isset( $error_messages[ $status ] ) ) {
                echo '<div class="notice notice-error is-dismissible"><p><strong>WP License Server:</strong> ' . esc_html( $error_messages[ $status ] ) . ' The key was stored in the database instead — use the "Auto-add to wp-config.php" button below when the file is writable.</p></div>';
            }

            ?>
            <div class="notice notice-warning" style="border-left-color:#f0ad4e">
                <p>
                    <strong>Welcome to WP License Server — Encryption Key Setup</strong>
                </p>
                <p style="font-size:13px;color:#555;margin:4px 0 0">
                    This plugin needs an encryption key to secure your license data.
                    Would you like the system to generate one and add it to your
                    <code>wp-config.php</code> file automatically?
                </p>
                <p style="margin:12px 0 4px">
                    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline">
                        ...
                        <button type="submit" class="button button-primary">Yes, generate and secure my key</button>
                    </form>
                    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline;margin-left:8px">
                        ...
                        <button type="submit" class="button button-secondary">No, I&#8217;ll do it manually</button>
                    </form>
                </p>
                <p style="color:#888;font-size:12px;margin:8px 0 0">
                    If you choose "No", the plugin will not be able to boot until
                    <code>WPLICENSE_ENCRYPTION_KEY</code> is defined in your
