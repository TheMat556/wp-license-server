<?php
/**
 * REST controller for POST /license-server/v1/chat/send.
 *
...
use WpLicenseServer\Rest\Dto\ChatSendRequest;
...
final class ChatSendController {
    ...
    public function handle( \WP_REST_Request $request ): \WP_REST_Response {
        $dto        = new ChatSendRequest( $request );
        ...
        $check = $this->feature_gate->require_feature( TierConfig::ROUTE_FEATURES['chat/send'], $license );
        if ( is_wp_error( $check ) ) {
            return self::error_response( $check );
        }

        $result = $this->chat_service->send_message(
            $license,
            $dto->domain,
