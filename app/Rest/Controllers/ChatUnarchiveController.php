<?php
/**
 * REST controller for POST /license-server/v1/chat/unarchive.
 *
...
use WpLicenseServer\Rest\Dto\ChatThreadActionRequest;
...
final class ChatUnarchiveController {
    ...
    public function handle( \WP_REST_Request $request ): \WP_REST_Response {
        $dto        = new ChatThreadActionRequest( $request );
        ...
        $check = $this->feature_gate->require_feature( TierConfig::ROUTE_FEATURES['chat/unarchive'], $license );
        if ( is_wp_error( $check ) ) {
            return self::error_response( $check );
        }

        $result = $this->chat_service->unarchive_thread(
            $license,
            $dto->domain,
