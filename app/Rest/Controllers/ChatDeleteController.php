<?php
/**
 * REST controller for POST /license-server/v1/chat/delete.
 *
...
final class ChatDeleteController {
    ...
    public function handle( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        $check = $this->feature_gate->require_feature( TierConfig::ROUTE_FEATURES['chat/delete'], $license );
        if ( is_wp_error( $check ) ) {
            return self::error_response( $check );
        }

        $result = $this->chat_service->delete_thread(
            $license,
            $dto->domain,
