#!/usr/bin/env bash
# Installs the WordPress PHPUnit test scaffolding.
# Adapted from the canonical WP-CLI script: https://make.wordpress.org/cli/handbook/misc/plugin-unit-tests/
...
recreate_db() {
	...
	if [[ $1 =~ ^(y|yes)$ ]]; then
		...
		echo "Leaving the existing test database ($DB_NAME) in place."
	fi
	shopt -u nocasematch
}
