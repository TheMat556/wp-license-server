...
describe('isEmbedMessage', () => {
  ...
  test.each([
    [
      'page-ready',
      {
        ...
        url: 'https://example.com/?wp_shell_embed=1',
      },
    ],
    ...
    [
      'breakout',
      {
        type: 'breakout',
        version: EMBED_MESSAGE_VERSION,
        url: 'https://example.com/wp-admin/page',
      },
    ],
  ...
  test('rejects wrong version', () => {
    expect(isEmbedMessage({ type: 'page-ready', version: 999, url: 'https://example.com/' })).toBe(
      false,
    );
  });
  ...
  test('rejects overlay-state with non-boolean isOpen', () => {
    expect(
      isEmbedMessage({ type: 'overlay-state', version: EMBED_MESSAGE_VERSION, isOpen: 'yes' }),
    ).toBe(false);
  });
