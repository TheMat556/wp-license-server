...
export function getWpNonce(): string {
  return (window as unknown as Record<string, Record<string, string>>)?.wpApiSettings?.nonce ?? '';
}

/** Build an absolute WP REST API URL for the given path. */
export function getRestUrl(path: string): string {
  const root: string =
    (window as unknown as Record<string, Record<string, string>>)?.wpApiSettings?.root ??
    '/wp-json/';
...
export function getAdminUrl(page: string, params: Record<string, string> = {}): string {
  const base =
    (window as unknown as Record<string, Record<string, string>>)?.wpApiSettings?.adminUrl ??
    '/wp-admin/admin.php';
